async function getSongs(folder) {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    if (href && href.endsWith(".mp3")) {
      songs.push(`./public/songs/${folder}${href}`); // correct full path
    }
  }
  // console.log(songs);
  ...
  for (const song of songs) {
    let songDetails = decodeURIComponent(song.split("/").pop())
      .replace(".mp3", "")
      .split(" - ");
    ...
    songUL.innerHTML += `<li class="flex">
                          ...
                          <div class="info">
                              ${songDetails[1] || "Unknown"}
                              <h5>${songDetails[0] || "Unknown"}</h5>
                          </div> 
                          </li>`;
  }
...
async function getAlbums() {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    if (href && !href.includes("public")) {
      albums.push(href); // only relative href
    }
  }
  // console.log(albums);
  ...
  for (const album of albums) {
    let cardName = decodeURIComponent(album.replace("/", ""));
    cardConatiner.innerHTML += `
      <div data-folder="${album}" class="card">
        <img src="./public/songs/${album}cover.jpg" alt="cover">
        <p>${cardName}</p>
...
async function addListeners() {
  ...
  prevBtn.addEventListener("click", () => {
    ...
    audio.volume =
      parseInt(document.querySelector(".volume input").value) / 100;
    playMusic();
  ...
  function addMusicInfo(index) {
    let songPath = songs[index];
    let fileName = decodeURIComponent(songPath.split("/").pop()); // Get only the filename
    let songDetails = fileName.replace(".mp3", "").split(" - ");
